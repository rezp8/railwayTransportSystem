    ...
    def log_in(self):
        while True:
            ...
            if user == self.username and pass0 == self.password:
                ...
                while True:
                    failed = input(
                        "1. Try Again / 2. Return to Previous Menu: ")
                    if failed == "1":
                        print("------------------------")
                        break
                    elif failed == "2":
        ...
        while True:
            ...
            while True:
                ...
                if not f.isalpha() or len(f) < 3:
                    print(
                        "Family name must contain only letters and be at least 3 characters long.")
                    continue
                break
            ...
            while True:
                print(
                    "Username must start with a letter and contain only letters and numbers (minimum 4 characters).")
            ...
            while True:
                print(
                    "Password must include letters, numbers, and one of the symbols '@' or '&'.")
                p = input("Password: ")
                if p == "0":
                    return
                if not re.match(r"^(?=.*[A-Za-z])(?=.*\d)(?=.*[@&])[A-Za-z\d@&]{6,}$", p):
                    print(
                        "Password must be at least 6 characters and include a letter, a number, and one of '@' or '&'.")
                    continue
                break

            new_dict = {"name": n, "family": f,
        ...
        while True:
            if not self.employees:
                ...
                for i, emp in enumerate(self.employees, 1):
                    print(
                        f"{i}. Name: {emp['name'].title()} | Family: {emp['family'].title()} | Email: {emp['email']} | Username: {emp['username']} | Password: {emp['password']}")
                    print("------------------------")
                escape = input("Enter '0' to return to the previous menu.")
                if escape == "0":
                    print("------------------------")
